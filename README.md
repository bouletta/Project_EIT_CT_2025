...
#### 2. Install *pygalmesh* ( but recommended)
...
The code expects a `Data_set/` directory containing the CT volumes and associated segmentation masks for each subject.  Each subject directory should contain `ct.nii.gz` (the CT volume) and a `segmentations/` folder with organ masks named as `<organ>.nii.gz`.  The segmentation file names follow the same naming convention as the Visible Human Project dataset (see comments in `CT_processing_functions.py`).
...
1. **Generate meshes** – Run `Example_meshing.ipynb` to process CT slices, extract torso and organ masks, and generate 2‑D finite‑element meshes with electrode locations.  This notebook demonstrates how to tune mesh resolution (`h` parameter) and other options.  The meshes and associated conductivity labels are saved in `Python_library/`.  MATLAB equivalents can be generated by converting the `.pkl` files to `.mat` using functions in `src_mat`.

2. **Load and simulate** – Run `Example_load_mesh.ipynb` to load a previously generated mesh, visualise it, assign realistic conductivity values to organs, solve the EIT forward problem using the built‑in CEM solver (see `EIT_sim.py`). It also demonstrates deformation of meshes between slices using thin‑plate splines.

3. **Matlab utilities** – Use `Example_driver.m` or `Driver_test_OOEIT.m` to call into the MATLAB functions.  These first one show how to load meshes saved from Python (as `.mat` files), generate electrode surfaces, smooth boundaries, and call MATLAB EIT solvers.The second one implement the solution by search (described in the report) and compare it with some other inverse problem solutions of OOEIT.

4. **Advanced reconstruction** – The `src_aatae/Driver_Reconstruct.py` script contains an example of searching through a library of mesh slices to find those whose simulated measurements best match given EIT data. The `Driver_for_matlab.py` script transform a set meshes from Python-readble file to matlab files.
...
- **Dataset** – You will need to supply your own CT NIfTI volumes and segmentation masks; these are not included due to size.  Ensure they are organised under `Data_set/<case_id>/` as described earlier.
...
- **Pygalmesh on Windows** – Building pygalmesh on Windows can be difficult due to CGAL/Eigen dependencies.  Consider running the notebooks in a Linux or WSL2 environment, or consult the [StackOverflow answer](https://stackoverflow.com/questions/61472028/how-does-one-install-pygalmesh-which-depends-on-eigen-on-a-windows-pc) for guidance.
